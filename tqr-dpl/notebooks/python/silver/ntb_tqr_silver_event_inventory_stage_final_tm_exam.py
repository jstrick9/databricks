...
#Job variables
job_name = 'ntb_tqr_silver_event_inventory_stage_final_tm_exam' 
...
try:
    df_event_inventory_stage_final = spark.sql("""
    ...
    (
       ...
       WHERE  trim(business_event_reason_cd) IN ('CNFRF','CNFRO','CNFRR','CNFRS','GNFRO','GNFRS','GNFRF')),

     Intervening24hrFinal AS
    (
       SELECT *
       FROM   ${stg_tqr_db}.src_trm_application_event
       WHERE  trim(business_event_reason_cd) IN ('CNFRF','CNFRO','CNFRR','CNFRS','GNFRO','GNFRS','GNFRF')  
       AND    cast(effective_ts AS DATE) BETWEEN date_sub(from_utc_timestamp(current_timestamp(),'America/New_York'),1) AND date_sub(from_utc_timestamp(current_timestamp(),'America/New_York'),2) ),
    ...
    (
           SELECT DISTINCT '101'          AS review_type_cd,
             ...
             join            sub_src_trm_application_event final_tm
             ON              app.serial_num_tx=final_tm.serial_num_tx
             join            filingbasis fb
             ...
             WHERE           NOT EXISTS
                                       (
                                              SELECT 1
                                              FROM   Intervening24hrFinal int24final_tm
                                              WHERE  int24final_tm.serial_num_tx = app.serial_num_tx )
             AND             final_tm.effective_ts <= date_sub(from_utc_timestamp(current_timestamp(),'America/New_York'),7)
             AND             final_tm.effective_ts >= to_date('${dataload_dt}')
            )SRC
    WHERE NOT EXISTS(
...
try:
    df_count = df_event_inventory_stage_final.count()
    df_event_inventory_stage_final.write.mode("append").format("delta").insertInto(f'{stg_tqr_db}.{trgt_tbl_name}')
...
# MAGIC %sql
# MAGIC select * from tqr.silver.job_log where job_nm = 'ntb_tqr_silver_event_inventory_stage_final_tm_exam' order by start_ts desc

# COMMAND ----------

spark.sql("""select * from tqr.silver.job_log where job_nm = 'ntb_tqr_silver_event_inventory_stage_final_tm_exam'  AND JOB_LOG_ID=23 order by start_ts desc""").show(truncate=False)

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from tqr.silver.job_control where job_nm = 'ntb_tqr_silver_event_inventory_stage_final_tm_exam' order by load_ts desc

# COMMAND ----------

# MAGIC %sql
# MAGIC select count(*) from tqr.silver.event_inventory_stage where review_type_cd = 101

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from tqr.silver.event_inventory_stage where review_type_cd = 101
