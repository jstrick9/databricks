...
#Job variables
job_name = 'ntb_tqr_silver_event_inventory_stage_sou' 
...
try:
    df_event_inventory_stage_sou = spark.sql("""
    ...
    (
           SELECT DISTINCT '103'        AS review_type_cd,
             ...
             join            ${stg_tqr_db}.src_trm_application_event sou
             ON              app.serial_num_tx=sou.serial_num_tx
             join            filingbasis fb
             ...
             WHERE           TRIM(sou.business_event_reason_cd) in ('CNPRP','CNSRP')
             AND             sou.effective_ts <= date_sub(from_utc_timestamp(current_timestamp(),'America/New_York'),2)
             AND             sou.effective_ts >= to_date('${dataload_dt}')
            )SRC
    WHERE NOT EXISTS(
...
try:
    df_count = df_event_inventory_stage_sou.count()
    df_event_inventory_stage_sou.write.mode("append").format("delta").insertInto(f'{stg_tqr_db}.{trgt_tbl_name}')
...
# MAGIC %sql
# MAGIC select * from tqr.silver.job_log where job_nm = 'ntb_tqr_silver_event_inventory_stage_sou' order by start_ts desc

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from tqr.silver.job_control where job_nm = 'ntb_tqr_silver_event_inventory_stage_sou' order by load_ts desc

# COMMAND ----------

# MAGIC %sql
# MAGIC select count(*) from tqr.silver.event_inventory_stage where review_type_cd = 103

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from tqr.silver.event_inventory_stage where review_type_cd = 103
