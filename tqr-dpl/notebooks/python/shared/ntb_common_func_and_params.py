...
#print(f'{common_config_file_path=},{mysql_lom_scope=},{oracle_tqr_scope=}')

# COMMAND ----------

# DBTITLE 1,Schema Variables
#schema variables
common_configs = read_yaml(common_config_file_path)
data_quality_db = common_configs['schema']['data_quality_catalog']
mysql_lom_scope = common_configs['secrets']['mysql_scope']
oracle_tqr_scope = common_configs['secrets']['oracle_trm_scope']
spark.sql(f"set data_quality_db = {data_quality_db}")

spark.conf.set('config.data_quality_db', data_quality_db.lower())
...
def begin_job_cntl(ctlg_db_name,job_name:str,job_start_ts):
    ...
    job_log_start_query = f"""
        ...
        (start_ts,end_ts,status_ct,record_qt,comment_tx) 
        values (
            from_utc_timestamp('{job_start_ts}','America/New_York'), null, 'started', 0, ''
            )
        """
...
def end_job_cntl(ctlg_db_name,job_name,job_start_ts,proc_stus_cd,df_count,error):
    
    if proc_stus_cd == 'completed':
        job_control_query = f"""
            insert into {ctlg_db_name}.job_control
            PARTITION (job_nm = '{job_name}')
            (load_ts,create_ts,create_user_id,last_mod_ts,last_mod_user_id)
            values ( from_utc_timestamp('{job_start_ts}','America/New_York'), from_utc_timestamp(current_timestamp(),'America/New_York'), 'etl', from_utc_timestamp(current_timestamp(),'America/New_York'),'etl') 
            """
...
def load_mysql_table_dsu(df_table,schema_name,mysql_tbl_name, saveMode):
...
def read_data_from_oracle_conn_dsu(sql_query: str, schema_name: str) -> DataFrame:
    """A common function to read data from mysql db"""
    
    pushdown_query = ("(" + sql_query + ") query_alias ")
    host = dbutils.secrets.get(scope=oracle_tqr_scope, key="host")
    port = dbutils.secrets.get(scope=oracle_tqr_scope, key="port")
    db_name = dbutils.secrets.get(scope=oracle_tqr_scope, key="db_name")
    ...
    try:
        df_data = (spark.read.format("jdbc")\
                          ...
                          .option("dbtable",pushdown_query )\
                          .option("user", dbutils.secrets.get(scope=oracle_tqr_scope, key="username"))\
                          .option("password", dbutils.secrets.get(scope=oracle_tqr_scope, key="password"))\
