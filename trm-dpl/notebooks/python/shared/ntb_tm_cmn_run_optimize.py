...
# MAGIC <pre>
# MAGIC Optimize
# MAGIC Optimizes the layout of Delta Lake data.
# MAGIC </pre>
...
# MAGIC schema_def = StructType([StructField('TABLE_GROUP_NAME', StringType(),False),StructField('TABLE_NAME', StringType(), True),StructField('FULL_LOAD', StringType(), True),StructField('DQ_FLTR', StringType(), True),StructField('LARGE_TABLE_IND', StringType(), True),StructField('CLUSTER', StringType(), True)])
...
schema_def = StructType([StructField('TABLE_GROUP_NAME', StringType(),False),StructField('TABLE_NAME', StringType(), True),StructField('FULL_LOAD', StringType(), True),StructField('DQ_FLTR', StringType(), True),StructField('LARGE_TABLE_IND', StringType(), True),StructField('CLUSTER', StringType(), True)])
...
for schema_metadata in list_schema_metadata:
    ...
    df_schema_metadata = df_schema_metadata.withColumn("CATALOG",f.split(f.lit(schema_metadata),'_')[0])
    df_schema_metadata_all = df_schema_metadata_all.unionByName(df_schema_metadata, allowMissingColumns=True)
df_schema_metadata_all = df_schema_metadata_all.select(f.lower("TABLE_NAME").alias("TABLE_NAME"),"CLUSTER","CATALOG")
...
for row in catalog_df.select("catalog").collect():
    ct = row["catalog"]
    print(f"Running Optimize on catalog: {ct}")
    ...
    for row in schema_df.select("databaseName").collect():
        ...
        table_df = table_df.withColumn("catalog",f.lit(ct))
        #table_df.display()
        job_control_df = table_df.alias("table_df").join(df_schema_metadata_all.alias("df_dq_fltr"),(f.col("table_df.tableName") == f.col("df_dq_fltr.TABLE_NAME")) & f.expr("table_df.catalog like (CONCAT('%' ,df_dq_fltr.CATALOG,'%'))"),"inner")
        cnt = 0
        for row in job_control_df.select("database","tableName","CLUSTER").collect():
            db, table, cluster = row["database"], row["tableName"], row["CLUSTER"]
            cmd = ("OPTIMIZE " +db+ "." +table+ ";")
