...
if (PROC_CTGRY_CD == 'SRC_TO_BRNZ'):
    ...
    for r in df_metadata.collect():
        ...
        job_nm = "ntb_"+src_name+"_"+tbl_nm.lower()+"_brnz_load"
        print(job_nm)
        try:
            print("\n")
            ...
            if fltr_col == '':
                ...
                #src_query =  f"""(select count(*) from {TRGT_SCHEMA}.{tbl_nm})"""
                src_query = f"""(select src_cnt from {trgt_catalog}.silver.job_log
                        where job_nm = '{job_nm}'
                        and status_ct = 'completed'
                        and end_ts = (select max(end_ts)  from {trgt_catalog}.silver.job_log
                        where job_nm = '{job_nm}'
                        and status_ct = 'completed'
                        group by job_nm)
                        )"""
            ...
            try:
                ...
                df_read_src_tbl = spark.sql(src_query)
