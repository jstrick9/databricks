...
# MAGIC table 
# MAGIC if not exists 
# MAGIC ${config.trgt_catalog}.silver.temp_summary_daily_ia_event_pull
# MAGIC (
...
# MAGIC )
# MAGIC using delta
# MAGIC location 's3://bdr-databricks-app-${config.dbx_env}/eds/delta_tables/${config.catalog}/silver/temp_summary_daily_ia_event_pull'
...
table_query = f"SELECT * FROM { src_db_name}.temp_summary_daily_ia_event"
...
df_src_cdc.write.mode("append").format("delta").insertInto(f'{trgt_catalog}.silver.temp_summary_daily_ia_event_pull')
