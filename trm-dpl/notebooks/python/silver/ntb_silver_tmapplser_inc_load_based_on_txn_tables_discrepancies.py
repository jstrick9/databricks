...
job_name = 'ntb_silver_tmapplser_inc_load_txn_discrepancies'
            ...
            FROM
              (
                SELECT
                  ...
                  AND  (0,fk_trademark_gid) {trademark_gid_str}
              )         
            """
...
from  {trm_catalog}.trademark th left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
    (trunc(th.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 OR trunc(th.create_ts) =  '{rdate}')
      AND (0,th.trademark_gid) {trademark_gid_str}
          """
...
df_insert2 = spark.sql(f"""
                       ...
                       'tmapplser' as create_user_id,
                      from_utc_timestamp(current_timestamp(),'America/New_York') as last_mod_ts,
                      'tmapplser' as last_mod_user_id,0 as lock_control_no
...
with
  b as  (
  select  mailing_address_gid , last_mod_ts, create_ts from {trm_catalog}.MAILING_ADDRESS
)
select    
         ...
         cast('{rdate}' as date) as pulldt ,
          'am_addr_h' as tabname
            from b 
LEFT JOIN {trm_catalog}.tm_mailing_addr ON   tm_mailing_addr.FK_MAILING_ADDRESS_GID  = b.MAILING_ADDRESS_GID
    INNER JOIN  (SELECT * FROM {trm_catalog}.TM_PARTY_ROLE WHERE FK_TM_PARTY_ROLE_CD = 'COR') TM_PARTY_ROLE  ON tm_mailing_addr.FK_TM_PARTY_ROLE_ID  = TM_PARTY_ROLE.TM_PARTY_ROLE_ID
    inner join  {trm_catalog}.trademark  th on  TM_PARTY_ROLE.fk_trademark_gid = th.trademark_gid
                                left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
     (trunc(b.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or trunc(b.create_ts)   = '{rdate}')
       AND (0, th.trademark_gid) {trademark_gid_str}
"""
...
SELECT
         ...
         regexp_substr(tc.fk_trademark_gid, '[^:]+$')   SER_NUM,
          ...
          'cl_h' as tabname
        FROM
            {trm_catalog}.tm_class tc inner join  {trm_catalog}.trademark  th on  tc.fk_trademark_gid = th.trademark_gid
                                left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
     (trunc(tc.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or trunc(tc.create_ts)   = '{rdate}')
       AND (0,th.trademark_gid) {trademark_gid_str}
          """
...
SELECT
          ...
          'cm_h' as tabname
         FROM
            {trm_catalog}.business_event be inner join  {trm_catalog}.trademark  th on  be.cfk_object_gid = th.trademark_gid
                                left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
...
df_insert6_query = f"""
 SELECT
          ...
          'fn_h' as tabname
 FROM
            {trm_catalog}.tm_foreign_basis fbh inner join  {trm_catalog}.trademark  th on  fbh.fk_trademark_gid = th.trademark_gid
                                left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
    (trunc(fbh.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or trunc(fbh.create_ts) = '{rdate}')
       AND (0,th.trademark_gid) {trademark_gid_str}
         """
...
from {trm_catalog}.tm_prior_registration pr
inner join {trm_catalog}.trademark th on th.trademark_gid =pr.fk_prior_trademark_gid
left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
    (trunc(pr.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or trunc(pr.create_ts) =  '{rdate}')
       AND (0,th.trademark_gid) {trademark_gid_str}
          """
...
SELECT      DISTINCT 'TX' as actcd,
          regexp_substr(TM_PARTY_ROLE.fk_trademark_gid, '[^:]+$')   SER_NUM,
          cast('{rdate}' as date) pulldt,
          'py_h' as tabname
FROM {trm_catalog}.INTERESTED_PARTY INNER JOIN {trm_catalog}.TM_PARTY_ROLE ON INTERESTED_PARTY.INTERESTED_PARTY_GID = TM_PARTY_ROLE.FK_INTERESTED_PARTY_GID
           inner join {trm_catalog}.trademark th on th.trademark_gid =TM_PARTY_ROLE .fk_trademark_gid
left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
    (trunc(INTERESTED_PARTY.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or trunc(INTERESTED_PARTY.create_ts)  = '{rdate}')
       AND (0,th.trademark_gid) {trademark_gid_str}
          """
...
VT_AF AS (
 ...
 , a.last_mod_ts, a.create_ts
FROM
  {trm_catalog}.USE_IN_ANOTHER_FORM a
...
VT_CU AS (
    ...
    STATEMENT_TX VT_TEXT, last_mod_ts, create_ts

    FROM {trm_catalog}.CONCURRENT_USE
),
VT_CS AS (
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'CS' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_CC AS (
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'CC' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
,
VT_CD AS (
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'CD' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_DM AS (
    SELECT  CAST(A.SERIAL_NUM_TX AS INTEGER) VT_SER_NUM, 'DM0000' VT_TEXT_TYPE , COALESCE(CAST(A.MARK_DESCRIPTION_TX AS VARCHAR(32767)),B.LITERAL_ELEMENT_TX, '')  AS VT_TEXT, A.last_mod_ts , A.create_ts
    FROM {trm_catalog}.TRADEMARK A LEFT JOIN  {trm_catalog}.TM_LITERAL B ON A.TRADEMARK_GID = B.FK_TRADEMARK_GID
),
VT_DO AS (
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'D0' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_D1 AS (
    ...
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'D1' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_IN AS (
    ...
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'IN' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_LS AS (
    ...
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'LS' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_NR AS (
    ...
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'NR' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_PM AS (
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'PM' || trim(to_char(SEQUENCE_NO, '0000')) VT_TEXT_TYPE, PSEUDO_MARK_TX VT_TEXT, last_mod_ts, create_ts
    FROM {trm_catalog}.TM_PSEUDO_MARK
),
VT_TF AS (
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'TF0000' VT_TEXT_TYPE, LIMITATION_TX VT_TEXT, last_mod_ts, create_ts
    FROM {trm_catalog}.SECTION_2F_STATEMENT
    WHERE LIMITATION_TX IS NOT NULL
),
VT_TR AS (
  
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'TR' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_TL AS (
    ...
    SELECT CAST(regexp_substr(fk_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'TL' || trim(to_char(ORDER_NO, '0000')) VT_TEXT_TYPE, STATEMENT_TX VT_TEXT, last_mod_ts, create_ts
...
VT_TN AS (
    ...
    SELECT CAST(regexp_substr(fk_parent_trademark_gid, '[^:]+$') AS INTEGER)  VT_SER_NUM, 'TNSFOO' VT_TEXT_TYPE,  regexp_substr(FK_RELATED_TRADEMARK_GID, '[^:]+$')  VT_TEXT, last_mod_ts, create_ts
...
FROM
  ...
  inner join {trm_catalog}.trademark th on th.serial_num_tx = vt.VT_SER_NUM
left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
    (trunc(VT.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or trunc(VT.create_ts)  = '{rdate}' )
       AND (0,th.trademark_gid) {trademark_gid_str}
...
select   DISTINCT 'TX' as actcd,
          ...
          'ri_h' as tabname
         from
TMINTLTM.INTERNATIONAL_REGISTRATION RI inner join TMINTLTM.INTERNATIONAL_REG_TM IRT ON IRT.FK_INTERNATIONAL_REG_GID = RI.INTERNATIONAL_REG_GID
 inner join {trm_catalog}.trademark th on th.serial_num_tx = IRT.DN_SERIAL_NUM
left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
 where   
    (trunc(RI.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or trunc(RI.create_ts) = '{rdate}')
       AND (0,th.trademark_gid) {trademark_gid_str}
          """
...
SELECT 
      DISTINCT 'TX' as actcd,
         ...
         cast('{rdate}' as date)  pulldt,
          'wp_h' as tabname
FROM
  {trm_catalog}.TM_DESIGN_ELEMENT tde inner join {trm_catalog}.trademark th on th.trademark_gid= tde.fk_trademark_gid
left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
where   
    ( trunc(tde.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 OR  trunc(tde.create_ts)  =  '{rdate}' )
       AND (0,th.trademark_gid) {trademark_gid_str}
          """
...
select
  DISTINCT 'IB' as actcd,
        ...
        cast('{rdate}' as date) pulldt,
          'mas_h' as tabname
FROM
  tmintltm.base_application bah  inner join {trm_catalog}.trademark th on th.trademark_gid= bah.cfk_trademark_gid
left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
where   
     ( trunc(bah.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or  trunc(bah.create_ts)  =  '{rdate}' )
       AND (0,th.trademark_gid) {trademark_gid_str}
          """
...
from  tmintltm.international_appl_event iae inner join  tmintltm.international_appl_evnt_rsn iaer on iae.international_appl_evnt_rsn_id = iaer.international_appl_evnt_rsn_id
      inner join  tmintltm.base_application bah on bah.FK_INTERNATIONAL_APPL_GID = iae.fk_international_appl_gid
       inner join {trm_catalog}.trademark th on th.trademark_gid= bah.cfk_trademark_gid
left join  (select * from {trm_catalog}.tm_milestone where fk_tm_milestone_cd ='REG' ) tmh on tmh.fk_trademark_gid  = th.trademark_gid
where   
    ( trunc(iae.last_mod_ts) between cast('{rdate}' as date) and cast('{rdate}' as date) + 1 or trunc(iae.create_ts)  = '{rdate}' )
       AND (0,th.trademark_gid) {trademark_gid_str}
          """
...
df_insert13 = df_insert12.withColumn("create_ts",from_utc_timestamp(current_timestamp(),'America/New_York'))\
    .withColumn("create_user_id", lit("tmapplser"))\
    .withColumn("last_mod_ts", from_utc_timestamp(current_timestamp(),'America/New_York'))\
    .withColumn("last_mod_user_id", lit("tmapplser"))\
    .withColumn("lock_control_no", lit("0")) 
...
# MAGIC %sql
# MAGIC create table if not exists ${config.trgt_catalog}.silver.TMAPPLSER_TXNS_MISSING_APPS
# MAGIC (
...
# MAGIC )
# MAGIC using delta
# MAGIC location 's3://bdr-databricks-app-${config.dbx_env}/eds/delta_tables/${config.trgt_catalog}/silver/tmapplser_txn_missing_apps'
...
try:
    ...
    df_tmapplser_inc_merge = spark.sql(f"""MERGE INTO {trgt_catalog}.silver.TMAPPLSER_TXNS_MISSING_APPS trgt
...
spark.sql(f"""
 CREATE VIEW IF NOT EXISTS {trgt_catalog}.silver.tmappl_daily_consolidated_vw AS
...
(
       SELECT 
              ...
              LAST_MOD_USER_ID,
              LOCK_CONTROL_NO,
              row_number() OVER (PARTITION BY SERNUM, PULLDT ORDER BY CREATE_TS DESC) AS ROW_NUM
       FROM {trgt_catalog}.silver.tmappl_daily_txn_hstry_vw
       ...
       SELECT 
              ...
              LOCK_CONTROL_NO,
              row_number() OVER (PARTITION BY SERNUM, PULLDT ORDER BY CREATE_TS DESC) AS ROW_NUM
       FROM {trgt_catalog}.silver.TMAPPLSER_TXNS_MISSING_APPS
       ...
       SELECT
              ...
              LOCK_CONTROL_NO,
              row_number() OVER (PARTITION BY SERNUM, PULLDT ORDER BY CREATE_TS DESC) AS ROW_NUM
       FROM {trgt_catalog}.silver.TMAPPLSER_HSTRY_MISSING_APPS
) 
WHERE ROW_NUM = 1
""")
...
notify = Notify()
templ_str = f'{SRC_SYS_NAME} : BDSS Txn Temp Summary Discrepancies Data Load Result'
