...
for table_name in dms_full_load_jobs_parameter_list: 
        try:
            print("\n")
            print(f"Performing delete for {table_name[3]}:")
            trgt_query =  f"""truncate table {trgt_catalog}.{database}.{table_name[3]}"""
...
spark.sql(f"""
update {trgt_catalog}.{database}.{control_table} 
set  initial_load_finished = false
""" +control_table_filter)
