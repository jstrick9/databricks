...
# MAGIC Catalogs come from config/{dbx_env}/tmngpdb-conf.yaml
# MAGIC
# MAGIC Output: https://www.uspto.gov/trademarks/application-timeline
...
case = (spark.table(f"{tmngpdb_catalog}.{silver_schema_name}.case_milestones")
  .filter("_is_current = true")
  .filter(F.col("filing_date") >= F.add_months(F.lit(snapshot_date), -lookback_months))
)
...
def wait_days_df(filing_col, processed_col, extra_filter=None):
    df = case.filter(F.col(processed_col).isNotNull() & F.col(filing_col).isNotNull())
...
if spark.table(f"{tmngpdb_catalog}.{gold_schema_name}.metric_targets").count() == 0:
    target_rows = [
      ...
      ("pre_exam_teas","TEAS","Pre-Examination Unit","days",10,100),
      ("pre_exam_madrid","MADRID","Pre-Examination Unit","days",10,110),
      ("esu_responses","Responses/Corrections","Examination Support Unit (ESU)","days",14,200),
      ("itu_extension","Extension requests","Intent to use","days",15,300),
      ("itu_sou","Statement of use","Intent to use","days",15,310),
      ("itu_divisional","Divisional requests","Intent to use","days",15,320),
      ("petitions_lop","Letters of protest","Petitions Office","days",60,400),
      ("postreg_affidavit","Affidavits of Use/Incontestability","Post Registration","days",90,500),
      ("postreg_renewal","Renewals","Post Registration","days",90,510),
      ("postreg_amendment","Amendments/Corrections","Post Registration","days",90,520),
    ]
    spark.createDataFrame(target_rows, ["metric_key","metric_name","section","unit","target_value","sort_order"])\
      .write.mode("overwrite").saveAsTable(f"{tmngpdb_catalog}.{gold_schema_name}.metric_targets")
...
# ---------- Build Gold output ----------
metrics_df = spark.createDataFrame(rows, ["metric_key","metric_name","section","unit","average_value","sample_size"])
...
out = (metrics_df.alias("m")
  ...
  .select(
    ...
    F.col("average_value"), F.col("target_value"),
