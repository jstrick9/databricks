...
# MAGIC trm_tmprodvty{env}.bronze.productivity_action_live
...
df_epquery_stg1_live = spark.sql(f"""
select 
    DISTINCT 
    EP.CFK_OBJECT_GID,
...
from trm_tmworker{env}.bronze.worker W
    LEFT JOIN (select distinct worker_no as worker_num, first(dn_worker_tm_organization_cd) over(partition by worker_no order by max(EP.transaction_effective_dt) desc) as current_lo
                from trm_tmworker{env}.bronze.worker W
                LEFT JOIN trm_tmprodvty{env}.bronze.production_transaction_live EP ON WORKER_NO = DN_WORKER_NO
                AND DELETE_IN = 'N' 
    ...
    LEFT JOIN trm_tmprodvty{env}.bronze.productivity_action_live PA 
...
LEFT JOIN trm_tmprodvty{env}.bronze.productivity_action_live PA1 ON FK_CORRECTED_PRODVTY_ACTN_ID = PA1.PRODUCTIVITY_ACTION_ID
  ...
  from
    (
      SELECT
        ...
        END status
      FROM
        trm_tmprodvty{env}.bronze.production_transaction_live EP1
        INNER JOIN trm_tmprodvty{env}.bronze.productivity_action_live PA2 ON FK_GENERATING_PRODVTY_ACTN_ID = PA2.PRODUCTIVITY_ACTION_ID --AND CFK_BCR_PAY_PERIOD_RANGE_NAME between 202201  and  substr(<Parameters.Pay Period>, 5,6)
        --AND DN_WORKER_NO > 49999
        AND DELETE_IN = 'N'
...
LEFT JOIN (
  ...
  from
    (
      ...
      FROM
        trm_tmprodvty{env}.bronze.production_transaction_live EP1
        INNER JOIN trm_tmprodvty{env}.bronze.productivity_action_live PA2 ON FK_GENERATING_PRODVTY_ACTN_ID = PA2.PRODUCTIVITY_ACTION_ID
        --AND DN_WORKER_NO > 49999
        AND DELETE_IN = 'N'
        AND PRODUCTIVITY_ACTION_CD IN (
          '6321',
...
df_epquery_stg1_live.write.mode("overwrite").format("delta").saveAsTable(f'trm_reporting{env}.silver.epquery_stg1_live')
...
df_epquery_stg2_live = spark.sql(f"""
SELECT
...
FROM
trm_reporting{env}.silver.epquery_stg1_live -- WHERE ((EP_PP_PERIOD between substr(<Parameters.Pay Period>, 5,4) || '01' and substr(<Parameters.Pay Period>, 5,6)
...
df_epquery_stg2_live.write.mode("overwrite").format("delta").saveAsTable(f'trm_reporting{env}.silver.epquery_stg2_live')
...
df_epquery_stg3_live = spark.sql(f"""
SELECT *,
...
FROM
  trm_reporting{env}.silver.epquery_stg2_live
  WHERE
...
df_epquery_stg3_live.write.mode("overwrite").format("delta").saveAsTable(f'trm_reporting{env}.silver.epquery_stg3_live')
