...
job_name = 'ntb_third_level_goods_services_incremental'
...
# 69 - 73
df_filings = df_filings.filter(col("filing_fy2") > (col("max_filing_fy") - lit(15)))
...
# 38
gs_sn_list = spark.sql(f"select * from {reporting_catalog}.silver.goods_services_sn_list")

# COMMAND ----------

# 39
df_39 = df_filings.join(gs_sn_list, "ser_num", "left_anti")

# COMMAND ----------

# 16
df_16 = df_60.join(df_39, "ser_num").drop("filing_fy", "city", "postal_cd", "ctry_nm", "filing_fy_month_int", "max_filing_fy")
...
df_38.write.mode("append").format("delta").insertInto(f"{reporting_catalog}.gold.goods_services_dashboard")
...
df_77.write.mode("append").format("delta").insertInto(f"{reporting_catalog}.silver.goods_services_sn_list")
