...
from utils.db_helpers import get_connection, read_yaml, validate_source_table
...
st.session_state["dbx_env"] = runtime_env
...
st.divider()
st.markdown("### <u>Source Validation Information</u>", unsafe_allow_html=True)
with st.expander("Click to View Source Info", expanded=False):
    if not validate_source_table(cursor, config):
        st.stop()
